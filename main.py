...
from src.idspy.core.pipeline import FitAwareObservablePipeline, ObservablePipeline, PipelineEvent
...
from src.idspy.events.subscribers.logging import Logger, Tracer, DataFrameProfiler
from src.idspy.services.setup import setup_logging
from src.idspy.steps.io import LoadData, SaveSplits
...
def main():
    ...
    schema.add(
        ["IN_BYTES", "IN_PKTS", "OUT_BYTES", "OUT_PKTS", "FLOW_DURATION_MILLISECONDS", "DURATION_IN", "DURATION_OUT",
         ...
         "TCP_WIN_MAX_IN", "TCP_WIN_MAX_OUT", "DNS_TTL_ANSWER"], ColumnRole.NUMERICAL)
    schema.add(
        ["L4_SRC_PORT", "L4_DST_PORT", "PROTOCOL", "L7_PROTO", "TCP_FLAGS", "CLIENT_TCP_FLAGS", "SERVER_TCP_FLAGS",
         "ICMP_TYPE", "ICMP_IPV4_TYPE", "DNS_QUERY_ID", "DNS_QUERY_TYPE"], ColumnRole.CATEGORICAL)
    ...
    main_pipeline = ObservablePipeline(
        steps=[
            LoadData(path="resources/data/dataset_v2/cic_2018_v2.csv", schema=schema, nrows=1000000),
            DropNulls(),
