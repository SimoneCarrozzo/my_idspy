...
from src.idspy.alt_federated_learning.steps.io.fed_saver import SaveFederatedData_alt
...
# )
from src.idspy.alt_federated_learning.core.fed_split import(
    IdentifyTopHosts_alt,
    SplitByHosts_alt,
    FederatedSplits_alt,
    AggregatedTestSet_alt,
    AnalyzeNonIID_alt,
    ApplyFitAwareToFederatedSplits_alt
)

from src.idspy.alt_federated_learning.orchestrator import FederatedTrainingRound_B_alt
from src.idspy.alt_federated_learning.steps.model.evaluator import EvaluateGlobalModel_alt
from src.idspy.alt_federated_learning.steps.metrics.visualization import FederatedPlotMetrics_alt, FederatedRoundMetrics_alt
from src.idspy.alt_federated_learning.steps.transform.fed_scale import (
    ComputeGlobalNormalizationStats_alt,
    ApplyGlobalNormalization_alt,
)
from src.idspy.alt_federated_learning.steps.transform.fed_map import (
    FrequencyMapGlobal_alt,
    LabelMapGlobal_alt,
    CreateOneVsRestLabels_alt
)
from src.idspy.alt_federated_learning.steps.model.evaluator import EvaluateClusterModels_alt
from src.idspy.alt_federated_learning.steps.metrics.visualization import ClusterRoundMetrics_alt
...
def main():
    ...
    FEDERATED_DATA_PATH = "c:/Users/simon/OneDrive/Documenti/TESI_UNI/SetUp/dataset_processati_federati/log7"
    LOGS_PATH = "c:/Users/simon/OneDrive/Documenti/TESI_UNI/SetUp/Modelli_Salvati/logs_federated/Log_to_final/log_pseudo-final4"
    
    # Parametri FL
    NUM_ROUNDS = 4#11 #DA            # Numero di round federati
    LOCAL_EPOCHS = 3#5 #DA 3-->4-->5          # Epoche locali per client
    ...
    if RUN_PREPROCESSING:
        ...
        fit_aware_pipeline = FitAwareObservablePipeline(
            steps=[
                #StandardScale(),           
                FrequencyMapGlobal_alt(max_levels=20, in_scope="data",
                    out_scope="data"),  
                LabelMapGlobal_alt(benign_tag="Benign",
                                # mapping_output_path=Path(FEDERATED_DATA_PATH) / "label_mapping.json", # 🆕 
                               in_scope="data",
                               out_scope="data"
                        ),
                CreateOneVsRestLabels_alt(
                                        attack_types=["DDOS attack-HOIC", "DoS attacks-Hulk", "Bot", "Infilteration", "DDoS attacks-LOIC-HTTP"],  
                                        in_scope="data",
                                        out_scope="data"
                                    ),
    ...
    if RUN_TRAINING:
        ...
        for round_num in range(start_round, NUM_ROUNDS):
            ...
            # Esegui un round
            round_step = FederatedTrainingRound_B_alt(
                round_num=round_num,
            ...
            if round_num % 2 == 0 and aggregated_test is not None:
                ...
                if not round_state.has('federated.cluster_specific_weights'):
                    ...
                    eval_cluster_step = EvaluateClusterModels_alt(batch_size=1024)
                    ...
                    cluster_metrics_step = ClusterRoundMetrics_alt(
        ...
        logger.info(f"{'='*70}")
        from src.idspy.alt_federated_learning.steps.metrics.visualization import ClusterPlotMetrics_alt
        # ✅ Generazione grafici finali
        cluster_plot_step = ClusterPlotMetrics_alt(
...
if __name__ == "__main__":
    ...
    if RUN_TRAINING:
        ...
        # NUOVA AGGIUNTA: 
        # Filtra host senza specializzazione (Benign Only)
        active_hosts = {
            host_id: splits 
            for host_id, splits in federated_splits.items()
            if host_specialization.get(host_id.replace('host_', '').replace('_', '.'), {}).get('target_column') is not None
        }

        logger.info(f"🎯 Host attivi per training: {len(active_hosts)}/{len(federated_splits)}")

        federated_splits = active_hosts  # Usa solo host con attacchi
        # <--- 🚀 NUOVA MODIFICA! --->
        # Ora che il ciclo for è finito, federated_splits è pieno. Filtriamolo.
        
        # 🔥 FILTRA CLIENT CON POCHI ATTACKS
        MIN_ATTACK_RATIO = 0.05  # Almeno 5% Attacks (flessibile)
        MIN_ATTACK_SAMPLES = 300  # O almeno 300 sample assoluti
        ...
        #     # 🎯 USA IL MAPPING ORIGINALE per i grafici!
        #     if 'original_label_map' in metadata:
        #         label_mapping = metadata['original_label_map']
                
        #         # Inverti mapping: {"Benign": 0, "DDoS": 1} → [0:"Benign", 1:"DDoS"]
        #         label_encoder_classes = [None] * len(label_mapping)
                
        #         for name, idx in label_mapping.items():
        #             label_encoder_classes[idx] = name
                
        #         logger.info(f"✅ Label encoder caricato: {len(label_encoder_classes)} classi")
        #         logger.info(f"   Classi: {label_encoder_classes}")
        #     else:
        #         logger.warning("⚠️ 'original_label_map' non trovato nei metadati!")
        #         label_encoder_classes = [f"Class_{i}" for i in range(15)]
        # else:
        #     logger.warning(f"⚠️ Metadata non trovato: {meta_path}")
        #     label_encoder_classes = [f"Class_{i}" for i in range(15)]
        ...
        if meta_path.exists():
           ...
           # ✅ CARICA SPECIALIZZAZIONE HOST
            host_specialization = metadata.get('host_specialization', {})
            # ✅ CARICA LABEL ENCODER (per grafici)
            if 'training_label_map' in metadata:
                label_mapping = metadata['training_label_map']
                label_encoder_classes = [None] * len(label_mapping)
                for name, idx in label_mapping.items():
                    label_encoder_classes[idx] = name
                logger.info(f"✅ Label encoder caricato: {label_encoder_classes}")
            else:
                label_encoder_classes = ["Benign", "Attack"]
            
            # Log specializzazione
            expert_counts = {}
            for ip, spec in host_specialization.items():
                target = spec.get('target_column')
                if target:
            ...
            logger.info(f"✅ Specializzazione: {len(host_specialization)} host")
        ...
        # ⚖️ CALCOLO PESI PER BILANCIAMENTO (Basato su statistics.json e test aggregato)
        # Sappiamo dai log: Benign ~424527, Attacks ~171049
        class_counts = np.array([424527, 171049])
        
        # Formula Radice Quadrata Inversa
        # weights = 1.0 / np.sqrt(class_counts)
        weights = 1.0 / class_counts
        
        
        # Normalizzazione: rendiamo il peso della classe maggioritaria (Benign) pari a 1.0
        normalized_weights = weights / weights[0] 
        
        device_weights = torch.FloatTensor(normalized_weights).to(device)
        
        logger.info(f"⚖️ Pesi classi calcolati (Sqrt Balanced): {normalized_weights}")
        logger.info(f"   (Il modello darà {normalized_weights[1]:.2f}x importanza in più agli attacchi)")
        
        # 🆕 Binary Cross Entropy con class weight
        from torch import nn
        loss_fn = nn.BCEWithLogitsLoss(
            pos_weight=torch.FloatTensor([normalized_weights[1]]).to(device)
        ).to(device)
        ...
        for round_num in range(start_round, NUM_ROUNDS):
            ...
            if round_num % 2 == 0 and aggregated_test is not None:
                ...
                # ✅ AGGIUNTA: Salva metriche dettagliate
                metrics_step = FederatedRoundMetrics(
                    log_dir=str(Path(LOGS_PATH) / "metrics"),
                    class_names=label_encoder_classes
                )
        ...
        # ✅ AGGIUNTA: Genera grafici finali
        plot_step = FederatedPlotMetrics(
            output_dir=str(Path(LOGS_PATH) / "plots"),
            rounds_history=round_history
        )

        plot_step.run(state=State({}))
