...
from src.idspy.real_fed_learn.server import Classic_FedServer, FederatedServer_C
...
#=============================================================# 
#=============================================================# 
class Classic_FedTraining(Step):
    """
    Esegue un singolo round di Federated Learning.
    ...
    def __init__(
        ...
        super().__init__(
            name=name or f"classic_federated_round_{round_num}",
            in_scope=in_scope,
            out_scope=out_scope,
        )
    ...
    def run(
        ...
        # ✅ AGGIUNGI QUESTO LOG:
        logger.debug(f"🔍 [Classic_FedTraining] Selected clients: {selected_clients}")
        ...
        for client_id in selected_clients:
            ...
            model_to_use = global_model
            logger.info(f"   🌐 {client_id} usa Global model (Classic FedAvg)")
            
            # ✅ AGGIUNGI QUESTI LOG:
            logger.debug(f"🔍 [Client {client_id}] Model device: {next(global_model.parameters()).device}")
            logger.debug(f"🔍 [Client {client_id}] Train samples: {len(client_train_data)}")
            ...
            # ✅ Aggiungi scheduler solo se esiste
            if state.has('federated.scheduler_config'):
                client_state.set('federated.scheduler_config', 
                                state.get('federated.scheduler_config', dict), dict)
        ...
        # ✅ AGGIUNGI DOPO IL LOOP (prima di "FILTRAGGIO AGGIORNAMENTI"):
        logger.debug(f"🔍 [Classic_FedTraining] Raccolti {len(client_updates)} updates totali")
        logger.debug(f"🔍 [Classic_FedTraining] Samples per client: {[(u['client_id'], u['num_samples']) for u in client_updates]}")
        ...
        # ✅ AGGIUNGI QUESTI LOG:
        logger.debug(f"🔍 [Classic_FedTraining] Invio {len(valid_updates)} updates al server")
        logger.debug(f"🔍 [Classic_FedTraining] Global model nel state: {state.has('federated.global_model')}")
        
        server_step = Classic_FedServer(aggregation_method="classic_fedavg")
        ...
        try:
            ...
            # ✅ AGGIUNGI QUESTO LOG:
            logger.debug(f"🔍 [Classic_FedTraining] Pesi aggiornati: {len(clean_state_dict)} layers")
            logger.debug(f"🔍 [Classic_FedTraining] Sample layer shape: {list(clean_state_dict.values())[0].shape}")
        ...
        round_metrics = {
            ...
            'acc_variance': np.var(client_accs) if client_accs else 0.0,
            'num_clusters': 1,
            'avg_cluster_size':  len(client_results),
        }
        ...
        # ✅ AGGIUNGI QUESTI LOG:
        logger.debug(f"🔍 [Classic_FedTraining] Round metrics keys: {list(round_metrics.keys())}")
        logger.debug(f"🔍 [Classic_FedTraining] Num clusters: {round_metrics['num_clusters']}")  # Deve essere 1
        logger.debug(f"🔍 [Classic_FedTraining] Final results keys: {list(final_results.keys())}")
