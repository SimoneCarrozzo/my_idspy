...
import pandas as pd
import numpy as np
from src.idspy.alt_federated_learning.client import FederatedClient_alt, FederatedClient_B_alt
from src.idspy.alt_federated_learning.server import FederatedServer_alt, FederatedServer_B_alt


logger = logging.getLogger(__name__)

""" VERSIONE DEL FED-TRAIN-ROUND DI TIPO A: OVVERO COLLABORA CON LA VERSIONE DEL FED-SERVER DI TIPO A E CON IL FED-CLIENT DI TIPO A. 
...
class FederatedTrainingRound_alt(Step):
    """
    ...
    @Step.requires(
        ...
        device=torch.device,
        client_attack_info=dict,  # ⬅️ AGGIUNGI
    ...
    def run(
        ...
        # Log info clustering (se disponibile)
        if len(client_to_cluster) > 0:
            logger.debug(f"   📊 Clustering attivo: {len(set(client_to_cluster.values()))} cluster")
            # logger.debug(f"   🔍 Esempio mapping: {list(client_to_cluster.items())[:3]}")
        else:
            logger.info(f"   🌐 Round 0: Nessun clustering (tutti usano global model)")
        ...
        for client_id in selected_clients:
            ...
            client_step = FederatedClient_alt(
                client_id=client_id,
            ...
            # ==== MODIFICA: Gestione specializzazione host (cluster) ====
            # 🆕 Assegna modello cluster-specific se disponibile
            cluster_id = client_to_cluster.get(client_id)
            
            #DEBUG-LOG
            logger.debug(f"   🔍 Debug: {client_id} → cluster_id = {cluster_id}")

            

            if cluster_id is not None and cluster_id in cluster_weights:
                # Crea copia modello con pesi del cluster
                cluster_model = copy.deepcopy(global_model)
                cluster_model.load_state_dict(cluster_weights[cluster_id])
                logger.info(f"   🎯 {client_id} usa Cluster {cluster_id} model")
                model_to_use = cluster_model
            else:
                # Primo round: usa modello globale
                model_to_use = global_model
                logger.info(f"   🌐 {client_id} usa Global model come primo round")
            # ==== FINE MODIFICA ====
        ...
        server_step = FederatedServer_alt(aggregation_method="fedavg")
        ...
        # #=======AGGIUNTO PER IL MOMENTO ==================
        # # 🆕 Log Clustering Info
        # if server_state.has('federated.cluster_info'):
        #     cluster_info = server_state.get('federated.cluster_info', dict)
        #     logger.info(f"\n🎯 Clustering Round {self.round_num}:")
        #     for cluster_id, info in cluster_info['cluster_aggregations'].items():
        #         logger.info(f"   Cluster {cluster_id}: {info['num_clients']} client")
        # #=======AGGIUNTO PER IL MOMENTO ==================
...
class FederatedTrainingRound_B_alt(Step):
    """
    Esegue un singolo round di Federated Learning.
    ...
    def run(
        ...
        for client_id in selected_clients:
            ...
            # Crea step di training per questo client
            client_step = FederatedClient_B_alt(
                client_id=client_id,
                local_epochs=self.local_epochs,
                batch_size=self.batch_size,
                device=device,
        ...
        server_step = FederatedServer_B_alt(aggregation_method="fedavg")
