...
def main():
    ...
    # schema.add(
    #     [# 🆕 Colonne IP per Federated Learning (NON sono feature, solo per split)
    #         "IPV4_SRC_ADDR",
    #         "IPV4_DST_ADDR",
    #     ],
    #     ColumnRole.FEATURES, # IGNORE = non usate come feature nel modello
    # )
    ...
    preprocessing_pipeline = ObservablePipeline(
        steps=[
            ...
            # 🆕 POI: Split federato per IP host
            # A questo punto i dati sono già trasformati (scalati, encoded)
            IdentifyTopHosts(
                num_hosts=10,
            ...
            FederatedSplits(
                ...
                val_size=0.15,
                test_size=0.15,
                stratify_column='Attack',  # Usa 'Attack' come target
                in_scope="federated",
                out_scope="federated"
            ),
            AnalyzeNonIID(
                label_col='Label',
                attack_col='Attack',  # Se hai una colonna con tipo di attacco
                in_scope="federated",
                out_scope="federated"
            ),
            ApplyFitAwareToFederatedSplits(
            ...
            SaveFederatedData(
                base_path="c:/Users/simon/OneDrive/Documenti/TESI_UNI/DataSets/processati/federated",
                # file_name="cic_2018_v2_federated",
                fmt="parquet",
            ),
            # SaveData(
    ...
    # 🆕 LOGGING RISULTATI FEDERATED SPLIT
    if state.has("federated.federated_splits"):
        federated_splits = state.get("federated.federated_splits", dict)
        logger.info("\n" + "="*70)
        logger.info("📡 FEDERATED SPLIT COMPLETATO")
        logger.info("="*70)
        logger.info(f"Numero di host: {len(federated_splits)}")
        for ip in federated_splits.keys():
            train_size = len(federated_splits[ip]['train'])
            val_size = len(federated_splits[ip]['val'])
            test_size = len(federated_splits[ip]['test'])
            logger.info(
                f"Host {ip}: train={train_size}, val={val_size}, test={test_size}"
            )
        
        # Test set aggregato
        if state.has("federated.aggregated_test"):
            agg_test = state.get("federated.aggregated_test", pd.DataFrame)
            logger.info(f"\n📊 Test set aggregato: {len(agg_test)} samples")
        
        # Analisi non-IID
        if state.has("federated.non_iid_analysis"):
            non_iid = state.get("federated.non_iid_analysis", pd.DataFrame)
            logger.info("\n📈 Analisi Non-IID:")
            logger.info(f"\n{non_iid.to_string()}")
    ...
    training_step = TrainWithEarlyStopping(
        ...
        num_epochs=1,              # Numero massimo di epoche
